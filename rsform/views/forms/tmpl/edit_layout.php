...
<fieldset>
	...
	<?php $xhtmlLayouts = array('inline-xhtml', '2lines-xhtml', 'responsive', 'responsive-uikit'); ?>
	<?php foreach ($xhtmlLayouts as $layout) {
		$image = $layout=='responsive-uikit'?$layout.'.png': $layout.'.gif';
		$title = $layout=='responsive-uikit'?'UI-kit': JText::_('RSFP_LAYOUT_'.str_replace('-', '_', $layout));
		?>
	<div class="rsform_layout_box">
		<label for="formLayout<?php echo ucfirst($layout); ?>">
			<input type="radio" id="formLayout<?php echo ucfirst($layout); ?>" name="FormLayoutName" value="<?php echo $layout; ?>" onclick="saveLayoutName('<?php echo $this->form->FormId; ?>', this.value);" <?php if ($this->form->FormLayoutName == $layout) { ?>checked="checked"<?php } ?> /><?php echo $title;?><br/>
			<img src="components/com_rsform/assets/images/layouts/<?php echo $image; ?>" width="175"/>
		</label>
	</div>
	<?php } ?>
	<span class="rsform_clear_both"></span>
